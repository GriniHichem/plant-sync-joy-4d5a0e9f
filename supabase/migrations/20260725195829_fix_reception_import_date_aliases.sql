-- Corrige la régression de mapping introduite lors de l'ajout du N° système.
-- L'interface envoie date/fournisseur/produit, tandis que la dernière RPC
-- attendait date_ticket/supplier_code/product_code.
-- Les deux conventions restent acceptées pour compatibilité.
...
BEGIN
  ...
  LOOP
    ...
    -- Réinitialisation explicite pour éviter qu'une ligne hérite des valeurs
    -- optionnelles de la ligne précédente.
    v_date := NULL;
    v_brut := NULL;
    v_net := NULL;
    v_abat := NULL;
    ...
    -- Parsing déterministe : ISO AAAA-MM-JJ ou format français JJ/MM/AAAA.
    -- Le format français est traité explicitement pour éviter l'inversion
    -- mois/jour dépendante du DateStyle PostgreSQL.
    IF s_date IS NOT NULL AND s_date ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' THEN
      BEGIN
        v_date := make_date(
          split_part(s_date, '-', 1)::int,
          split_part(s_date, '-', 2)::int,
          split_part(s_date, '-', 3)::int
        );
      EXCEPTION WHEN OTHERS THEN
        v_date := NULL;
      END;
    ELSIF s_date IS NOT NULL AND s_date ~ '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}$' THEN
      BEGIN
        v_date := make_date(
          split_part(s_date, '/', 3)::int,
          split_part(s_date, '/', 2)::int,
    ...
    -- Accepte aussi bien les codes que les libellés envoyés par l'interface.
