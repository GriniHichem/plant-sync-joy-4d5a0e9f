...
BEGIN
  IF NOT (public.has_role(auth.uid(),'admin')
       OR public.has_role(auth.uid(),'responsable_qualite')
       OR public.has_role(auth.uid(),'controleur_qualite')
       OR public.has_role(auth.uid(),'agreeur')) THEN
...
GRANT EXECUTE ON FUNCTION public.import_reception_poids_bruts(jsonb) TO authenticated;
