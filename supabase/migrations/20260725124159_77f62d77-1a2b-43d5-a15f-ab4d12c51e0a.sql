-- 1) Storage policies: allow agréeur to upload/delete reception photos
DROP POLICY IF EXISTS "reception photos upload" ON storage.objects;
CREATE POLICY "reception photos upload" ON storage.objects
FOR INSERT TO authenticated
WITH CHECK (
  bucket_id = 'reception-photos' AND (
    has_role(auth.uid(), 'admin'::app_role) OR
    has_role(auth.uid(), 'responsable_si'::app_role) OR
    has_role(auth.uid(), 'directeur_qualite'::app_role) OR
    has_role(auth.uid(), 'responsable_controle_qualite'::app_role) OR
    has_role(auth.uid(), 'controleur_qualite'::app_role) OR
    has_role(auth.uid(), 'agreeur'::app_role)
  )
);

DROP POLICY IF EXISTS "reception photos delete" ON storage.objects;
CREATE POLICY "reception photos delete" ON storage.objects
...
-- 2) Import CSV : mode 'replace' préserve les photos (UPDATE sur place)
...
DECLARE
  r jsonb;
  idx int := 0;
  imported int := 0; replaced int := 0; skipped int := 0; updated_photos int := 0;
  errors jsonb := '[]'::jsonb;
  ...
  FOR r IN SELECT * FROM jsonb_array_elements(COALESCE(rows,'[]'::jsonb)) LOOP
    ...
    IF existing_id IS NOT NULL THEN
      IF on_conflict = 'ignore' THEN
        ...
        CONTINUE;
      ELSE
        -- REPLACE = UPDATE en place : préserve id + photos + orientations liées
        ...
        -- Pesée : upsert (une seule pesée par ticket usuellement)
        ...
        ELSE
          ...
          ) VALUES (existing_id, v_brut, v_abat, auth.uid(), weighed_ts);
        END IF;
