...
DROP POLICY IF EXISTS "QOIO viewable by authenticated" ON public.quality_of_indicator_overrides;
CREATE POLICY "QOIO viewable by authenticated"
ON public.quality_of_indicator_overrides FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "QOIO manage by quality/methodes/prod" ON public.quality_of_indicator_overrides;
CREATE POLICY "QOIO manage by quality/methodes/prod"
...
DROP TRIGGER IF EXISTS trg_qoio_updated_at ON public.quality_of_indicator_overrides;
CREATE TRIGGER trg_qoio_updated_at BEFORE UPDATE ON public.quality_of_indicator_overrides
FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

-- Plan de contrôle effectif d'un OF : héritage + ajouts locaux - retraits locaux
...
RETURNS TABLE (
  ...
  effective_frequency_minutes integer,
...
AS $$
  WITH of_ctx AS (
    SELECT o.id AS of_id, o.product_id, o.recipe_id, o.line_id, p.family_id
  ...
  candidates AS (
    ...
    -- Ajouts locaux propres à l'OF (dérogation ponctuelle)
    SELECT
      qi.id, qi.code, qi.name, qi.description,
      qi.indicator_type::text, qi.category::text,
      qi.unit, qi.target_value, qi.min_value, qi.max_value,
      ...
      (o.is_required OR qi.is_required),
      (o.is_blocking OR qi.is_blocking),
      'of'::text, 6,
      o.id
    FROM public.quality_of_indicator_overrides o
    JOIN public.quality_indicators qi ON qi.id = o.indicator_id
    WHERE o.of_id = p_of_id AND o.mode = 'add' AND qi.is_active = true
  ),
  ranked AS (
    SELECT c.*,
  ...
  SELECT
    ...
    r.effective_frequency_type, r.effective_frequency_minutes, r.effective_is_required, r.effective_is_blocking,
    r.match_scope, r.assignment_id
  FROM ranked r
  WHERE r.rn = 1
    AND NOT EXISTS (
      SELECT 1 FROM public.quality_of_indicator_overrides x
      WHERE x.of_id = p_of_id AND x.indicator_id = r.indicator_id AND x.mode = 'exclude'
    )
