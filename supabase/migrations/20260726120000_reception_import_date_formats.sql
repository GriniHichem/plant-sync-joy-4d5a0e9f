-- Import tickets : parsing des dates robuste (JJ/MM/AAAA, AAAA-MM-JJ, variantes).
...
BEGIN
  ...
  LOOP
    ...
    IF s_date ~ '^[0-9]{4}[-/][0-9]{1,2}[-/][0-9]{1,2}$' THEN
      BEGIN
        v_date := make_date(
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 1)::int,
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 2)::int,
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 3)::int
        );
      EXCEPTION WHEN OTHERS THEN
        v_date := NULL;
      END;
    ELSIF s_date ~ '^[0-9]{1,2}[-/][0-9]{1,2}[-/][0-9]{4}$' THEN
      BEGIN
        v_date := make_date(
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 3)::int,
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 2)::int,
      ...
      EXCEPTION WHEN OTHERS THEN
        v_date := NULL;
      END;
    ELSIF s_date ~ '^[0-9]{1,2}[-/][0-9]{1,2}[-/][0-9]{2}$' THEN
      BEGIN
        v_date := make_date(
          2000 + split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 3)::int,
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 2)::int,
          split_part(regexp_replace(s_date, '[-/]', '-', 'g'), '-', 1)::int
        );
      EXCEPTION WHEN OTHERS THEN
        v_date := NULL;
      END;
    ELSIF s_date ~ '^[0-9]{8}$' THEN
      BEGIN
        v_date := make_date(
          substr(s_date, 1, 4)::int, substr(s_date, 5, 2)::int, substr(s_date, 7, 2)::int
        );
      EXCEPTION WHEN OTHERS THEN
        v_date := NULL;
      END;
