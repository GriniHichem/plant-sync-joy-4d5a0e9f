-- Suppression des fonctions existantes pour permettre le changement de signature des paramètres
...
BEGIN
    SELECT json_build_object(
        'total_tickets', (SELECT count(*) FROM public.reception_tickets WHERE created_at BETWEEN p_start_date AND p_end_date),
        'clotures', (SELECT count(*) FROM public.reception_tickets WHERE created_at BETWEEN p_start_date AND p_end_date AND statut = 'cloture'),
        'a_peser', (SELECT count(*) FROM public.reception_tickets WHERE created_at BETWEEN p_start_date AND p_end_date AND etat_pesee = 'a_peser'),
        'poids_total_net', COALESCE((SELECT sum(poids_net_kg) FROM public.v_reception_global WHERE created_at BETWEEN p_start_date AND p_end_date), 0),
        'moy_abattement', COALESCE((SELECT avg(taux_abattement) FROM public.reception_tickets WHERE created_at BETWEEN p_start_date AND p_end_date AND taux_abattement IS NOT NULL), 0),
        'moy_duree', COALESCE((SELECT avg(EXTRACT(EPOCH FROM (cloture_at - created_at)) / 60) FROM public.reception_tickets WHERE created_at BETWEEN p_start_date AND p_end_date AND cloture_at IS NOT NULL), 0)
    ) INTO result;
