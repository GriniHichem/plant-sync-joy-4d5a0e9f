...
-- SUJET : Réception F&L – Correction du calcul de la durée moyenne dans les KPIs globaux (Consultation)
-- La durée moyenne doit être calculée en minutes selon la règle : (heure_fin - heure_debut) + (24h si fin < debut).
-- On exclut les tickets sans heure_debut ou heure_fin.
...
AS $function$
  ...
  durations AS (
    SELECT 
      *,
      (
        CASE 
          WHEN heure_fin < heure_debut THEN (heure_fin + interval '24 hours') - heure_debut
          ELSE heure_fin - heure_debut
        END
  ...
  SELECT jsonb_build_object(
    ...
    'clotures', (SELECT count(*)::integer FROM durations), -- Compte ceux qui ont une durée calculable
...
GRANT EXECUTE ON FUNCTION public.get_reception_kpis(date, date, uuid, uuid, uuid, text, text, text, timestamp, timestamp) TO authenticated;
