...
BEGIN
  ...
  WHERE (cloture_by = p_user_id::text OR created_by = p_user_id::text)
    AND statut = 'cloture'
    AND (cloture_at::timestamp with time zone) >= p_start_time
    AND (cloture_at::timestamp with time zone) < p_end_time;
END;
$$;
