  ...
  const { data: refs } = useQuery({
    ...
    queryFn: async () => {
      const [lines, products, suppliers, campaigns] = await Promise.all([
        (supabase.from("production_lines").select("id, designation").eq("is_active" as any, true) as any).order("designation"),
        (supabase.from("products").select("id, designation").eq("is_active" as any, true) as any).order("designation").limit(500),
        (supabase.from("reception_suppliers").select("id, nom").eq("actif" as any, true) as any).order("nom").limit(500),
